...
app.post('/contact', async (req, res) => {
  ...
  const transporter = nodemailer.createTransport({
      ...
      auth: {
          ...
          pass: process.env.EMAIL_PASS, // 발신자 비밀번호 (앱 비밀번호)
      },
  });

  // 이메일 옵션 설정
  const mailOptions = {
      from: email, // 사용자가 입력한 이메일
      to: process.env.EMAIL_RECEIVER, // 수신자 이메일 (환경 변수)
      subject: `문의사항: ${name}`,
      text: `이름: ${name}\n이메일: ${email}\n\n메시지:\n${message}`,
  };
  ...
  try {
      ...
      res.status(500).json({ success: false, message: `문의 전송 중 오류가 발생했습니다: ${error.message}` });
  }
});
